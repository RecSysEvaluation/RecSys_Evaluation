...
class SessionDataIterator:
    ...
    def test_data(self, enable_neg_samples, reset_hook=None):
        batch_size = self.batch_size
        iters = np.arange(batch_size)
        maxiter = iters.max()
        start = self.offset_sessions[self.session_idx_arr[iters]]
        ...
        while not finished:
            # Find the last index of each session
            last_indices = end - 1  # Last item index in each session
            last_items = torch.tensor(self.data_items[last_indices], requires_grad=False, device=self.device)
            
            yield last_items  # Yield only the last item of each session

            finished_mask = np.ones(batch_size, dtype=bool)  # Mark all sessions as finished
            n_finished = finished_mask.sum()
...
class GRU4Rec:
    def __init__(self, layers=[100], loss='cross-entropy', batch_size=64, dropout_p_embed=0.0,
        ...
        """
        temp = [self.device, self.layers, self.loss, self.elu_param, self.logq, self.batch_size, self.dropout_p_embed, self.dropout_p_hidden, 
                self.learning_rate, self.momentum, self.sample_alpha, self.constrained_embedding, self.n_epochs]
    ...
    def fit(self, data, sample_cache_max_size=10000000, compatibility_mode=True, item_key='ItemId', session_key='SessionId', time_key='Time'):
        ...
        self.data_iterator = SessionDataIterator(data, self.batch_size, n_sample=self.n_sample, sample_alpha=self.sample_alpha, 
                                                 sample_cache_max_size=sample_cache_max_size, item_key=item_key, session_key=session_key, time_key=time_key, 
                                                 session_order='time', device=self.device)
